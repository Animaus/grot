			...
			<td valign="top" style="width: 50%;">
				<table border="1" style="width: 100%;">
					...
					<tr>
						...
						<td valign="top" rowspan="2"><c:if test="${user.hasRole('ROLE_ADMIN') == true}"><sft:checkbox path='member.enabled' value='${enabled}' />&nbsp;</c:if>Actief</td>
					</tr>
					<tr>
						<td valign="top">&nbsp;</td>
					</tr>
					<tr>
						...
						<td valign="top">
							<c:if test="${user.hasRole('ROLE_ADMIN') == false}">${member.userRoleNames}</c:if><c:forEach items='${roles}' var='role'><c:set var="checked" value="false" /><c:forEach items="${member.userRoles}" var="userRole"><c:if test="${role.roleName==userRole.roleName}"><c:set var="checked" value="true" /></c:if></c:forEach><c:choose><c:when test="${checked==true}"><input name="roles" type="checkbox" value="${role.roleName}" checked />&nbsp;${role.roleDesc}<br />
