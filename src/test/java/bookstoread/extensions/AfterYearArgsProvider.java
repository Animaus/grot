...
public class AfterYearArgsProvider implements ArgumentsProvider {
	...
	public Stream<? extends Arguments> provideArguments(ExtensionContext context) throws Exception {
		...
		int year = 2007;
		return Stream.of(Arguments.of(BookPublishedYearFilter.After(year), effectiveJava),
				Arguments.of(BookPublishedYearFilter.After(year), codeComplete),
				Arguments.of(BookPublishedYearFilter.After(year), mythicalManMonth),
				Arguments.of(BookPublishedYearFilter.After(year), cleanCode),
				Arguments.of(BookPublishedYearFilter.After(year), designPatterns));
	}
}
